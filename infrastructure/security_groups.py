import pulumi
import pulumi_aws as aws

# Load configuration
config = pulumi.Config()

def create_security_groups(vpc):
    alb_port = config.require_int("alb_port")
    
    # Create a Security Group for the ALB
    alb_security_group = aws.ec2.SecurityGroup("alb-security-group",
        vpc_id=vpc.id,
        description=f"Allow inbound traffic",
        ingress=[
            aws.ec2.SecurityGroupIngressArgs(
                protocol="tcp",
                from_port=alb_port,
                to_port=alb_port,
                cidr_blocks=["0.0.0.0/0"],
            ),
        ],
        egress=[
            aws.ec2.SecurityGroupEgressArgs(
                protocol="-1",
                from_port=0,
                to_port=0,
                cidr_blocks=["0.0.0.0/0"],
            ),
        ],
        tags={
            "Name": "alb-security-group",
        },
    )

    fargate_port = config.require_int("fargate_port")
    # Create a Security Group for the Fargate tasks
    fargate_security_group = aws.ec2.SecurityGroup("fargate-security-group",
        vpc_id=vpc.id,
        description="Allow inbound traffic from the ALB",
        ingress=[
            aws.ec2.SecurityGroupIngressArgs(
                protocol="tcp",
                from_port=fargate_port,
                to_port=fargate_port,
                security_groups=[alb_security_group.id],
            ),
        ],
        egress=[
            aws.ec2.SecurityGroupEgressArgs(
                protocol="-1",
                from_port=0,
                to_port=0,
                cidr_blocks=["0.0.0.0/0"],
            ),
        ],
        tags={
            "Name": "fargate-security-group",
        },
    )

    return {
        "alb_security_group": alb_security_group,
        "fargate_security_group": fargate_security_group,
    }